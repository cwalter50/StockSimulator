...
struct PersistenceController {
    ...
    static var preview: PersistenceController = {
        ...
        for i in 0..<10 {
            ...
            newWatchlist.name = "Sample\(i)"
            newWatchlist.created = Date()
            for j in 0..<5 {
                let newStock = Stock(context: viewContext)
                newStock.symbol = "TEST\(j)"
                newStock.regularMarketChange = 1.03
                newStock.regularMarketChangePercent = 0.5
                newStock.regularMarketPrice = 89.25
                newWatchlist.addToStocks(newStock)
